...
def ExtractPaper(img, mask):
    ...
    if isinstance(kpoints, np.ndarray):
        ...
        print("Warning: No valid document corners detected")
        return img, None 

def ScannSavedImage(fname, scanner, gray_paper = False):
    org = cv2.imread(fname)
    if org is None:
        raise FileNotFoundError(f"Could not load image: {fname}")
    ...
    # Only draw box if corners were detected
...
def EnhancePaper(img):
    if isinstance(img, np.ndarray):
        # Only enhance if it's a grayscale image
        if len(img.shape) == 2:
        ...
        return img  # Return original image if it's not grayscale
    return img
...
def SaveCompImage(fname, org, paper):
    ...
    # # Handle case where paper is same size as original
    # if paper.shape == org.shape:
    #     if len(paper.shape) == 2:  # If paper is grayscale
    #         paper_rgb = cv2.cvtColor(paper, cv2.COLOR_GRAY2RGB)
    #     else:
    #         paper_rgb = paper
    #     base[0:size_[0], size_[1]:size_[1]*2] = paper_rgb
    # else:
    #     # Original offset calculation for transformed paper
    #     offset = (np.array(size_[:2]) - np.array(paper.shape[:2])) // 2
    #     if len(paper.shape) == 2:  # If paper is grayscale
    #         paper_rgb = cv2.cvtColor(paper, cv2.COLOR_GRAY2RGB)
    #     else:
    #         paper_rgb = paper
    #     base[offset[0]:offset[0]+paper.shape[0], 
    #          size_[1]+offset[1]:size_[1]+offset[1]+paper.shape[1]] = paper_rgb

    # Convert to RGB if the paper is grayscale
    if len(paper.shape) == 2:
        paper_rgb = cv2.cvtColor(paper, cv2.COLOR_GRAY2RGB)
    else:
        paper_rgb = paper
        
    # Save only the processed paper image
    cv2.imwrite(fname, paper_rgb)
